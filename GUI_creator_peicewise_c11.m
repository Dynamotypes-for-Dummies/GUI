...
fig = uifigure('Position', [20, 50, 1500, 750], 'Name', 'Class 11', 'Color',[1,1,1]);
Height = 735;
% A slider
%%change caption here 
...
Height = Height - mindip;
%%change k value here
kkSlider = uislider(fig, 'Position', [50,Height, 500, 3], 'Limits', [0.00, 0.1], 'Value', 0.001);
...
%%change title
titleLabel = uilabel(fig, 'Position', [830, 700, 500, 40], 'Text', 'Class 11 - Parameter Control and Visualization', 'FontSize', 16, 'FontWeight', 'bold');
   %change description
descriptionLines = {

    'Class 11 seizures have a supercritical Hopf (SupH) onset bifurcation and a supercritical Hopf (SupH)  offset bifurcation. The path';
    '';
...
filename = sprintf('Class_11_timeseries_Offset_index_%d_Onset_index_%d_k_%.2f_dynamical_noise_%.2f_acquisition_noise_%.2f_tMax_%.2f_adjusted_frequency_%.2f_electrode_drift_filter_%.2f.mat', ...
        BIndex, AIndex, k, sigma, acq_noise,  frequency, drift);
else

filename = sprintf('Class_11_timeseries_Offset_index_%d_Onset_index_%d_k_%.2f_dynamical_noise_%.2f_acquisition_noise_%.2f.mat', ...
